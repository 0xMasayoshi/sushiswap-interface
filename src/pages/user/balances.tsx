...
import { CurrencyAmount, WNATIVE } from '@sushiswap/sdk'
...
const TokenBalance = ({ tokenBalance }: { tokenBalance: BentoBalance }) => {
  ...
  const walletBalanceUSDC = Number(wallet?.toFixed(token.decimals)) * Number(tokenPriceUSDC?.toFixed(18))
  const bentoBalanceUSDC = Number(bento?.toFixed(token.decimals)) * Number(tokenPriceUSDC?.toFixed(18))
    ...
    <Paper className="space-y-4">
      ...
      >
        ...
        <div className="flex items-center justify-end">
          <div>
            <div className="text-right">{formatNumber(wallet.toFixed(token.decimals))} </div>
            <div className="text-right text-secondary">{formatNumber(walletBalanceUSDC, true)}</div>
          </div>
        </div>
        <div className="flex items-center justify-end">
          <div>
            <div className="text-right">{formatNumber(bento.toFixed(token.decimals))} </div>
...
export function Deposit({ tokenBalance }: { tokenBalance: BentoBalance }): JSX.Element {
  ...
  const [approvalState, approve] = useApproveCallback(
    CurrencyAmount.fromRawAmount(token, value.toBigNumber(token.decimals).toString()),
    ...
    <>
      {account && (
        <div className="pr-4 mb-2 text-sm text-right cursor-pointer text-secondary">
          {i18n._(t`Wallet Balance`)}: {formatNumber(wallet.toFixed(token.decimals))}
        </div>
      )}
      <div className="relative flex items-center w-full mb-4">
        ...
        {account && (
          <Button
            ...
            size="xs"
            onClick={() => {
              setValue(wallet.toFixed(token.decimals))
            }}
      ...
      {!showApprove && (
        <Button
          ...
          onClick={async () => {
            setPendingTx(true)
            await deposit(token.address, value.toBigNumber(token.decimals))
            setPendingTx(false)
          }}
        >
          {i18n._(t`Deposit`)}
...
function Withdraw({ tokenBalance }: { tokenBalance: BentoBalance }): JSX.Element {
  ...
  return (
    <>
      {account && (
        <div className="pr-4 mb-2 text-sm text-right cursor-pointer text-secondary">
          {i18n._(t`Bento Balance: ${formatNumber(bento.toFixed(token.decimals))}`)}
        </div>
      )}
      <div className="relative flex items-center w-full mb-4">
        <NumericalInput
        ...
        {account && (
          <Button
            ...
            size="xs"
            onClick={() => {
              setValue(bento.toFixed(token.decimals))
            }}
      ...
      <Button
        ...
        onClick={async () => {
          setPendingTx(true)
          await withdraw(token.address, value.toBigNumber(token.decimals))
          setPendingTx(false)
        }}
      >
        {i18n._(t`Withdraw`)}
